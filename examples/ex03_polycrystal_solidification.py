...
sim.write_extended_xyz(filename, atom_coord, [atom_data[:,0], atom_data[:,1], atom_data[:,2]], ['den', 'ene', 'pf'], simulation_time=0.0)   # Save atom data to an extended XYZ file, gzip compressed by default
...
for step in range(nstep):
    ...
    if np.mod(step+1,nout)==0 or step+1==nstep:
        ...
        if np.mod(step+1,n_save_step_data)==0:
            ...
            sim.save_pickle(filename, [ step, total_time, ndiv, domain_size, den, state_output[:state_output_idx+1,:] ])

            # Save data to file
            # =================
            plotnr   = str(step+1).zfill(nfill)
            filename = output_path + 'pfc_data_' + plotnr     
            sim.write_extended_xyz(filename, atom_coord, [atom_data[:,0], atom_data[:,1], atom_data[:,2]], ['den', 'ene', 'pf'], simulation_time=total_time)   # Save atom data to an extended XYZ file, gzip-compressed by default
